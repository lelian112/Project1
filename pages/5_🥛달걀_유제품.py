...
def get_chatbot_response1(chatbot_response):
    ...
    for i, source in enumerate(chatbot_response['source_documents']):
        ...
        with open(f'./m_data/images/egg_dairy/{n}.jpg', 'rb') as file:
            contents = file.read()
...
db = Chroma( persist_directory='./DB/egg-dairy',  embedding_function=embedding)
...
def main():
  ...
  #채팅 공간
  if "messages_egg" not in st.session_state:
    st.session_state['messages_egg'] = []

  #초기화 버튼
  if 'check_reset_egg' not in st.session_state:
     st.session_state['check_reset_egg'] = False
    
  # Set page title
  st.set_page_config(
      page_title="달걀/유제품",
      layout="wide",
      page_icon= ":glass_of_milk:",
  )

  st.title("Ready for Egg-Dairy Cooding!")
  st.markdown('---')
  st.subheader(''' 주 메뉴인 달걀/유제품으로 요리를 해봐요! 레디포 쿠딩이 빠르게 도와줄거예요  
TIP! 달걀/유제품을 빼고 입력해도 달걀/유제품과 어울리는 레시피를 추천해 줄거에요 😊 
''')
  ...
  if st.sidebar.button(label = '초기화'):
      st.session_state['messages_egg'] = []
      st.session_state['check_reset_egg'] = True
  # st.sidebar.markdown('---')
      
# -----------------------------------------------------------#
  # Display chat messages from history on app rerun
  for message in st.session_state['messages_egg']:
  ...
  if user_input := st.chat_input("말씀해주세요."):
      ...
      st.session_state['messages_egg'].append({"role": "user", "content": user_input})
      ...
      st.session_state['messages_egg'].append({"role": "assistant", "content": response1})
