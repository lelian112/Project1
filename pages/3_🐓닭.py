...
def get_chatbot_response(chatbot_response):
    ...
    for i, source in enumerate(chatbot_response['source_documents']):
        ...
        with open(f'./m_data/images/chicken/{n}.jpg', 'rb') as file:
            contents = file.read()
...
db = Chroma( persist_directory='./DB/chicken',  embedding_function=embedding)
...
def main():
  ...
  #채팅 공간
  if "messages_chick" not in st.session_state:
    st.session_state['messages_chick'] = []

  #초기화 버튼
  if 'check_reset_chick' not in st.session_state:
     st.session_state['check_reset_chick'] = False
  ...
  st.title("Ready for Chicken Cooding!")
  st.markdown('---')
  st.subheader(''' 주 메뉴인 닭으로 요리를 해봐요! 레디포 쿠딩이 빠르게 도와줄거예요  
TIP! 닭을 빼고 입력해도 닭과 어울리는 레시피를 추천해 줄거에요 😊 
''')
  ...
  if st.sidebar.button(label = '초기화'):
      st.session_state['messages_chick'] = []
      st.session_state['check_reset_chick'] = True
  # st.sidebar.markdown('---')
      
# -----------------------------------------------------------#
  # Display chat messages from history on app rerun
  for message in st.session_state['messages_chick']:
  ...
  if user_input := st.chat_input("말씀해주세요."):
      ...
      st.session_state['messages_chick'].append({"role": "user", "content": user_input})
      ...
      st.session_state['messages_chick'].append({"role": "assistant", "content": response1})
