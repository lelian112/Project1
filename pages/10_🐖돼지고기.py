...
db = Chroma( persist_directory='./DB/pork',  embedding_function=embedding)
...
def main():
  ...
  if "messages_pork" not in st.session_state:
    st.session_state['messages_pork'] = []

  #초기화 버튼
  if 'check_reset_pork' not in st.session_state:
     st.session_state['check_reset_pork'] = False
  ...
  st.title("Ready for Pork Cooding!")
  st.markdown('---')
  st.subheader(''' 주 메뉴인 돼지고기로 요리를 해봐요! 레디포 쿠딩이 빠르게 도와줄거예요  
TIP! 돼지고기를 빼고 입력해도 돼지고기와 어울리는 레시피를 추천해 줄거에요 😊 
''')
  ...
  if st.sidebar.button(label = '초기화'):
      st.session_state['messages_pork'] = []
      st.session_state['check_reset_pork'] = True
  # st.sidebar.markdown('---')
      
# -----------------------------------------------------------#
  # Display chat messages from history on app rerun
  for message in st.session_state['messages_pork']:
  ...
  if user_input := st.chat_input("말씀해주세요."):
      ...
      st.session_state['messages_pork'].append({"role": "user", "content": user_input})
      ...
      st.session_state['messages_pork'].append({"role": "assistant", "content": response1})
