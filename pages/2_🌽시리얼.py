...
def get_chatbot_response1(chatbot_response):
    ...
    for i, source in enumerate(chatbot_response['source_documents']):
        ...
        with open(f'./m_data/images/cereal/{n}.jpg', 'rb') as file:
            contents = file.read()
...
db = Chroma( persist_directory='./DB/cereal',  embedding_function=embedding)
...
def main():
  ...
  if "messages_cereal" not in st.session_state:
    st.session_state['messages_cereal'] = []

  #초기화 버튼
  if 'check_reset_cereal' not in st.session_state:
     st.session_state['check_reset_cereal'] = False
  ...
  st.title("Ready for Cereal Cooding!")
  st.markdown('---')
  st.subheader(''' 주 메뉴인 시리얼로 요리를 해봐요! 레디포 쿠딩이 빠르게 도와줄거예요  
TIP! 시리얼을 빼고 입력해도 시리얼과 어울리는 레시피를 추천해 줄거에요 😊 
''')
  ...
  if st.sidebar.button(label = '초기화'):
      st.session_state['messages_cereal'] = []
      st.session_state['check_reset_cereal'] = True
      
# -----------------------------------------------------------#
  # Display chat messages from history on app rerun
  for message in st.session_state['messages_cereal']:
  ...
  if user_input := st.chat_input("말씀해주세요."):
      ...
      st.session_state['messages_cereal'].append({"role": "user", "content": user_input})
      ...
      st.session_state['messages_cereal'].append({"role": "assistant", "content": response})
