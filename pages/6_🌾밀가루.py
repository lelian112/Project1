...
def get_chatbot_response1(chatbot_response):
    ...
    for i, source in enumerate(chatbot_response['source_documents']):
        ...
        with open(f'./m_data/images/flour/{n}.jpg', 'rb') as file:
            contents = file.read()
...
db = Chroma( persist_directory='./DB/flour',  embedding_function=embedding)
...
def main():
  ...
  #채팅 공간
  if "messages_flour" not in st.session_state:
      st.session_state['messages_flour'] = []

  #초기화 버튼
  if 'check_reset_flour' not in st.session_state:
     st.session_state['check_reset_flour'] = False
  ...
  st.title("Ready for Flour Cooding!")
  st.markdown('---')
  st.subheader(''' 주 메뉴인 밀가루로 요리를 해봐요! 레디포 쿠딩이 빠르게 도와줄거예요  
TIP! 밀가루를 빼고 입력해도 밀가루와 어울리는 레시피를 추천해 줄거에요 😊 
''')
  ...
  if st.sidebar.button(label = '초기화'):
      st.session_state['messages_flour'] = []
      st.session_state['check_reset_flour'] = True
  # st.sidebar.markdown('---')
      
# -----------------------------------------------------------#
  # Display chat messages from history on app rerun
  for message in st.session_state['messages_flour']:
  ...
  if user_input := st.chat_input("말씀해주세요."):
      ...
      st.session_state['messages_flour'].append({"role": "user", "content": user_input})
      ...
      st.session_state['messages_flour'].append({"role": "assistant", "content": response1})
