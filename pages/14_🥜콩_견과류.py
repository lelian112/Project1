...
def get_chatbot_response1(chatbot_response):
    ...
    for i, source in enumerate(chatbot_response['source_documents']):
        ...
        with open(f'./m_data/images/soy_nuts/{n}.jpg', 'rb') as file:
            contents = file.read()
...
db = Chroma( persist_directory='./DB/soy_nuts',  embedding_function=embedding)
...
def main():
  ...
  #채팅공간
  if "messages_soynuts" not in st.session_state:
      st.session_state['messages_soynuts'] = []

  #초기화 버튼
  if 'check_reset_soynuts' not in st.session_state:
     st.session_state['check_reset_soynuts'] = False
  ...
  st.title("Ready for Nuts Cooding!")
  st.markdown('---')
  st.subheader(''' 주 메뉴인 콩/견과류으로 요리를 해봐요! 레디포 쿠딩이 빠르게 도와줄거예요  
TIP! 콩/견과류를 빼고 입력해도 콩/견과류와 어울리는 레시피를 추천해 줄거에요 😊 
''')
  ...
  if st.sidebar.button(label = '초기화'):
      st.session_state['messages_soynuts'] = []
      st.session_state['check_reset_soynuts'] = True
  # st.sidebar.markdown('---')
      
# -----------------------------------------------------------#
  # Display chat messages from history on app rerun
  for message in st.session_state['messages_soynuts']:
  ...
  if user_input := st.chat_input("말씀해주세요."):
      ...
      st.session_state['messages_soynuts'].append({"role": "user", "content": user_input})
      ...
      st.session_state['messages_soynuts'].append({"role": "assistant", "content": response1})
